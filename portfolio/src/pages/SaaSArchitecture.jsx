...
const techStack = [
  ...
  { name: 'Clerk / Auth.js', level: 88 },
  { name: 'Prisma', level: 85 },
  { name: 'Vercel', level: 90 },
  { name: 'PostHog', level: 80 },
]
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 100, marginBottom: 20 }}>
              <Shield size={16} color="#6CB4EE" /><span style={{ fontSize: 13, fontWeight: 600, color: '#6CB4EE' }}>SaaS Architecture</span>
            </div>
            <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 'clamp(36px, 5vw, 56px)', fontWeight: 800, color: '#1A1A2E', lineHeight: 1.15, marginBottom: 16 }}>
              Production-Grade <br />
              <span style={{ background: 'linear-gradient(135deg, #6CB4EE, #E84393)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>SaaS Products</span>
            </h1>
            <p style={{ fontSize: 18, color: '#4A4A68', lineHeight: 1.8, maxWidth: 600 }}>Building complete SaaS products with authentication, billing, multi-tenancy, and everything needed to launch.</p>
          </motion.div>
        </div>
      </section>
              ...
              {techStack.map((tech, i) => (
                <motion.div key={tech.name} initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ width: '100%', height: 8, background: '#EFF6FF', borderRadius: 4, overflow: 'hidden' }}>
                    <motion.div initial={{ width: 0 }} whileInView={{ width: `${tech.level}%` }} viewport={{ once: true }} transition={{ duration: 1.2, delay: i * 0.1, ease: 'easeOut' }} style={{ height: '100%', background: 'linear-gradient(90deg, #6CB4EE, #E84393)', borderRadius: 4 }} />
                  </div>
                </motion.div>
              ))}
            </div>
