        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', background: '#FFF0F6', border: '1px solid #F8C8DC', borderRadius: 100, marginBottom: 20 }}>
              <Palette size={16} color="#E84393" /><span style={{ fontSize: 13, fontWeight: 600, color: '#E84393' }}>UI/UX Design</span>
            </div>
            <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 'clamp(36px, 5vw, 56px)', fontWeight: 800, color: '#1A1A2E', lineHeight: 1.15, marginBottom: 16 }}>
              Beautiful <span style={{ background: 'linear-gradient(135deg, #E84393, #FD79A8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>User Interfaces</span>
            </h1>
            <p style={{ fontSize: 18, color: '#4A4A68', lineHeight: 1.8, maxWidth: 600 }}>Designing intuitive, beautiful interfaces with focus on user experience, accessibility, and delightful interactions.</p>
          </motion.div>
        </div>
      </section>
      <section style={{ padding: '80px 24px', background: '#FFFFFF' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'start' }} className="tech-grid-sub">
            ...
            <div>
              <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 32, fontWeight: 700, color: '#1A1A2E', marginBottom: 24 }}>Tools</h2>
              {techStack.map((tech, i) => (
                <motion.div key={tech.name} initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ width: '100%', height: 8, background: '#FFF0F6', borderRadius: 4, overflow: 'hidden' }}>
                    <motion.div initial={{ width: 0 }} whileInView={{ width: `${tech.level}%` }} viewport={{ once: true }} transition={{ duration: 1.2, delay: i * 0.1, ease: 'easeOut' }} style={{ height: '100%', background: 'linear-gradient(90deg, #E84393, #FD79A8)', borderRadius: 4 }} />
                  </div>
                </motion.div>
              ))}
            </div>
